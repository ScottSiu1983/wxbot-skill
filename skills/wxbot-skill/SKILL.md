...
通过本地 OCR + Quartz HID 事件控制 WeChat 桌面应用。所有识别在本机完成，支持像素级结构化布局探测与拟人化仿真输入。

## 脚本路径

```
skills/wxbot-skill/scripts/wechat.py
```
...
### 读取聊天上下文
```bash
python3 skills/wxbot-skill/scripts/wechat.py chat read <name>
```
输出示例:
```
[OK] Kent (5 msgs):
...
### 回复聊天
```bash
python3 skills/wxbot-skill/scripts/wechat.py chat reply <name> "<message>"
```
...
**`chat read` 和 `chat reply` 必须在后台运行**，因为脚本通过 Quartz 控制键盘鼠标，前台运行时会干扰终端。
...
2. **默认按上下文自行拟写回复** — 用户下达模糊指令时，应读完上下文后自行拟写。
3. **前缀自动添加** — 所有回复由脚本根据 `reply_prefix` 自动加前缀，AI Agent 不需要写。
4. **极简口语化原则** — 回复应像真人一样简短、口语化，严禁总结式、汇报式的长句。
5. **点对点回复** — 只针对当前上下文中一个具体的、最新的观点进行回复。
6. **你来我往** — 保持"你一言我一语"的快节奏交流感，不要长篇大论。
7. **不确定时必须确认** — 输出 `[需要确认]: 请问您希望如何回复 <name>？` 并等待用户指示
