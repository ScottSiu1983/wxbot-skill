...
│  ├── chat_reply()          → 发送回复 (带 auto_send 控制)       │
│  │                                                              │
│  ├── _detect_absolute_layout() → 核心：像素级高精度布局分析      │
│  ├── _parse_messages()      → 核心：多模态消息与引用解析        │
│  └── _navigate_to_chat()    → 鲁棒导航 (Search + OCR 验证)      │
├─────────────────────────────────────────────────────────────────┤
│  local_vision.py          │  computer_use.py                    │
│  (Vision OCR & 视觉工具)   │  (pyautogui & 仿真互动)             │
│  ├── find_text()          │  ├── click()                        │
│  ├── get_screen_text()    │  ├── type_text() (拟人化分块)        │
...
系统通过扫描截图数组中的 RGB 突变来锁定分界线：

#### 2.2.1 纵向分界线 (v_line_x)
- **扫描逻辑**: 在窗口中间选取 `1/4`, `1/2`, `3/4` 三个 Y 轴高度作为采样点 (`scan_y_samples`)。
- **算法**: 从窗口中心向右偏移，寻找颜色差异 `np.sum(abs(curr - left)) > 15` 的点。
- **校验**: 执行垂直贯穿校验，要求 `matches / total > 0.7` 的点都在同一 X 轴线上。
- **含义**: 该线区分了左侧的“聊天列表/侧边栏”与右侧的“消息内容区”。

#### 2.2.2 横向分界线 (h_lines)
- **扫描逻辑**: 在消息流泳道内选取 3 个 X 轴中心点。
- **算法**: 向上扫描 Y 轴，寻找颜色差异 `> 15` 的横线。
- **校验**: 水平贯穿校验 `matches / total > 0.85`。
- **含义**: 解析出标题栏下缘、消息流与输入框的分界。
...
为了防止点击坐标“穿透”到背景窗口，搜索结果的交互已从“盲选坐标”改为“精准定位匹配项”：

1. **全屏扫描**: 在搜索结果面板出现后，使用 `accurate` 模式识别联系人/群组文字。
2. **局部匹配**:
    - 排除 Y < 90px 的搜索框干扰。
    - 循环校验 `name` 是否包含在 OCR 文本中（去空格、不区分大小写）。
3. **安全打开**: 找到目标坐标后执行点击，随后强制执行 `_verify_chat_open()` 确保窗口切换成功。
...
- **判定**: 基于连通域色彩方差 `20 <= std < 45` 且形状呈横向长方形。
- **关联**: 若 OCR 文字座落在该引用气泡范围内，则将其标记为 `[引用: ...]`。

### 6.3 发送人归属逻辑

- **1:1 模式**: 根据 X 轴是否过中线直接判断。
- **群聊模式**: 
    - 优先定位头像 (`avatars`)。
    - 判定规则：`is_me = (avatar_x > midline + 50)`。
    - 消息内容与其上方的最近头像进行关联。
